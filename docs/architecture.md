...
- Usage loads once on page entry and is updated from every success or quota response, with no polling. Reset time is displayed in Pakistan time. Anonymous and Google exhaustion use the exact owner-directed message; the verified public WhatsApp action is built only from `academy_settings/default` values and a properly encoded `wa.me` URL.
