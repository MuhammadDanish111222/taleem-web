...
export function ContentBrowser({
  ...
  const [searchResults, setSearchResults] = useState<PublicResourceDto[] | null>(null);
  ...
  // Execute search when debouncedQuery >= 2 chars
  useEffect(() => {
    if (!boardId || !classId || debouncedQuery.length < 2) {
  ...
  }, [boardId, classId, subjectId, resourceType, debouncedQuery]);
    ...
    <div className="space-y-8 max-w-6xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      ...
      <section className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
        ...
        <div className="pt-4 border-t border-gray-100">
          <label className="block text-xs font-medium text-gray-700 mb-1">Search Resources</label>
          <div className="relative">
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              disabled={!boardId || !classId}
              placeholder={
                !boardId || !classId
                  ? "Select Board and Class to enable search"
                  : "Search title by token or prefix (e.g. physics, math)..."
              }
              className="w-full text-sm border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 p-2.5 border pr-10 disabled:bg-gray-50 disabled:cursor-not-allowed"
            />
            {isSearching && (
              <div className="absolute right-3 top-2.5 w-4 h-4 border-2 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
            )}
          </div>
        </div>
      </section>
          ...
          <div className="space-y-4">
            ...
            {searchResults.length === 0 ? (
              <div className="bg-white p-12 text-center rounded-xl shadow-sm border border-gray-100">
                <p className="text-gray-500">No resources found matching &quot;{debouncedQuery}&quot;.</p>
              </div>
            ) : (
