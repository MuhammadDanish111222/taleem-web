...
const multipleAskWorkflowStatus = z.enum([
  ...
  "invalid", "too_many_questions", "cancelled", "limit_reached",
]);
...
export const multipleAskCorrectionBrowserRequestSchema = z.object({
  ...
  if (value.answerMode === "mcq") {
    if (labels.join("") !== "ABCD") {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "INVALID_MCQ_OPTIONS" });
    }
  } else if (labels.length > 0) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "OPTIONS_ONLY_FOR_MCQ" });
  }
});
...
export function toInternalCorrectionRequest(input: z.infer<typeof multipleAskCorrectionBrowserRequestSchema>) {
  return {
    request_id: input.requestId,
    question_text: input.questionText,
    answer_mode: input.answerMode,
    mcq_options: input.mcqOptions.map((option) => ({ label: option.label, text: option.text })),
  };
}

export function toInternalResumeRequest(input: z.infer<typeof multipleAskResumeBrowserRequestSchema>) {
  return { request_id: input.requestId };
}

export function toBrowserStatusResponse(input: z.infer<typeof multipleAskStatusInternalResponseSchema>) {
  return {
    jobId: input.job_id, workflowStatus: input.workflow_status, inputKind: input.input_kind,
    ...
    items: input.items.map((item) => ({
      ...
      mcqOptions: item.mcq_options, unclearReason: item.unclear_reason, terminalErrorCode: item.terminal_error_code,
      sourceLocator: item.source_locator, extractionVersion: item.extraction_version,
      correctionVersion: item.correction_version, correctedAt: item.corrected_at,
      result: item.result ? {
        answerSource: item.result.answer_source, blocks: item.result.blocks,
        citations: item.result.citations, visualIds: item.result.visual_ids,
