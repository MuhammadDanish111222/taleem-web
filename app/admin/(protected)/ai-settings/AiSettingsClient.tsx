...
type Effective = { key: string; scope: { kind: ScopeKind }; value: unknown };
...
export default function AiSettingsClient() {
  ...
  const [key, setKey] = useState(""); const [scope, setScope] = useState<ScopeKind>("global");
  ...
  const load = useCallback(async () => { const response = await fetch("/api/admin/ai-settings", { cache: "no-store" }); if (!response.ok) { setMessage("Settings are unavailable."); return; } const data = await response.json(); setRegistry(data.registry); setEffective(data.effective); setKey((selected) => selected || data.registry[0]?.key || ""); }, []);
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); if (!current) return; const selectedScope: Record<string, string> = { kind: scope }; if (scope === "subject" || scope === "class_subject") selectedScope.subject_id = subjectId; if (scope === "class_subject") selectedScope.class_id = classId; if (scope === "account_tier") selectedScope.account_tier = tier; const response = await fetch("/api/admin/ai-settings", { method: "POST", headers: { "content-type": "application/json", "x-csrf-token": await csrf() }, body: JSON.stringify({ key, scope: selectedScope, value: valueFor() }) }); const data = await response.json().catch(() => ({})); setMessage(response.ok ? "Setting saved. The next relevant request will use it." : `Setting rejected: ${data.code ?? "RUNTIME_SETTING_REJECTED"}`); if (response.ok) await load(); };
  return <section className="p-8 max-w-4xl"><h1 className="text-2xl font-bold">AI runtime settings</h1><p className="mt-2 text-sm text-gray-600">Only allowlisted operational settings are editable. Prompts remain in Ask Prompts; the grounded-evidence rule and all secrets stay locked in code or environment configuration.</p><form onSubmit={submit} className="mt-6 grid gap-4 rounded border bg-white p-5">
    ...
    {scope === "account_tier" && <label>Tier <select className="ml-2 border p-2" value={tier} onChange={(e) => setTier(e.target.value)}><option>anonymous</option><option>google</option><option>premium</option></select></label>}
    ...
    <button className="w-fit rounded bg-blue-700 px-4 py-2 text-white" type="submit">Save setting</button>{message && <p role="status" className="text-sm">{message}</p>}</form>
    <h2 className="mt-8 text-lg font-semibold">Current global values</h2><ul className="mt-2 text-sm">{effective.map((item) => <li key={item.key}>{item.key}: {String(item.value)} ({item.scope.kind})</li>)}</ul></section>;
}
