...
export default function CandidateReviewClient() {
  ...
  return (
    <section className="mx-auto max-w-7xl p-6 text-slate-900">
      ...
      <h1 className="text-3xl font-bold">Generated candidate review</h1>
      <p className="mt-2 text-slate-600">Generated answers remain pending and are never trusted until an admin validates and corrects the complete revision.</p>

      <div className="my-6 grid gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-4">
        ...
        <label className="flex items-center gap-2 text-sm">At least
        ...
        <button type="button" onClick={() => void loadCandidates()} disabled={busy} className="rounded-lg bg-indigo-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Load pending</button>
      </div>

      <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="font-semibold text-amber-950">Candidate retention dry run</h2>
            <p className="text-sm text-amber-900">Preview expired pending/rejected rows before any separately authorized bounded cleanup. This control never deletes data.</p>
          </div>
          <button type="button" onClick={() => void previewRetention()} disabled={busy} className="rounded-lg bg-amber-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Preview retention</button>
        </div>
        {retentionPreview ? (
      ...
      <div className="grid gap-6 lg:grid-cols-[22rem_1fr]">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <h2 className="mb-3 font-semibold">Pending candidates ({items.length})</h2>
          <div className="grid max-h-[54rem] gap-2 overflow-y-auto">
            {items.map((item) => (
              <button key={item.id} type="button" onClick={() => void inspect(item.id)} className={`rounded-lg border p-3 text-left text-sm ${selected?.id === item.id ? "border-indigo-500 bg-indigo-50" : "border-slate-200"}`}>
                <span className="font-semibold">{item.subject_id} · {item.answer_mode}</span>
                <span className="mt-1 block text-xs text-slate-500">{item.board_id}/{item.class_id}{item.chapter_id ? `/${item.chapter_id}` : ""}</span>
            ...
            {!busy && items.length === 0 ? <p className="text-sm text-slate-500">No matching pending candidates loaded.</p> : null}
          </div>
        </div>

        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          {!selected ? <p className="text-slate-500">Select a pending candidate to inspect it safely.</p> : (
            <div className="grid gap-6">
              <div>
                <h2 className="text-xl font-semibold">Original candidate</h2>
                <dl className="mt-3 grid gap-2 rounded-lg bg-slate-50 p-4 text-sm md:grid-cols-2">
                  <div><dt className="font-medium">Answer source</dt><dd>{selected.answer_source}</dd></div>
                  <div><dt className="font-medium">Provider/model</dt><dd>{selected.provider ?? "not recorded"} / {selected.model ?? "not recorded"}</dd></div>
                  <div><dt className="font-medium">Mode/style</dt><dd>{selected.answer_mode} / {selected.answer_style}</dd></div>
                  <div><dt className="font-medium">Created</dt><dd>{new Date(selected.created_at).toLocaleString()}</dd></div>
                  <div><dt className="font-medium">Source feature</dt><dd>{selected.source_feature}</dd></div>
                  <div><dt className="font-medium">Prompt version</dt><dd className="break-all font-mono text-xs">{selected.prompt_version ?? "not recorded"}</dd></div>
                  <div><dt className="font-medium">Corpus version</dt><dd className="break-all font-mono text-xs">{selected.corpus_version_id ?? "none (General AI)"}</dd></div>
                </dl>
                <h3 className="mt-4 text-sm font-semibold">Original question</h3>
                <p className="mt-1 whitespace-pre-wrap rounded-lg border border-slate-200 p-3 text-sm">{selected.raw_question}</p>
                <h3 className="mt-4 text-sm font-semibold">Original blocks and provenance references</h3>
                <pre className="mt-1 max-h-80 overflow-auto whitespace-pre-wrap rounded-lg bg-slate-950 p-4 text-xs text-slate-100">{JSON.stringify({
              ...
              <div className="border-t border-slate-200 pt-6">
                <h2 className="text-xl font-semibold">Corrected approved revision</h2>
                <p className="mb-4 mt-1 text-sm text-slate-500">Review every field. Approval creates an immutable trusted revision, links this candidate, clears pending retention, and queues approved-only embeddings.</p>
                <ApprovedQuestionEditor value={draft} onChange={setDraft} idPrefix="candidate-approval" />
                <button type="button" onClick={() => void approve()} disabled={busy} className="mt-4 rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Approve corrected revision</button>
              </div>

              <div className="border-t border-slate-200 pt-6">
                <h2 className="text-xl font-semibold text-red-800">Reject candidate</h2>
                <textarea aria-label="Required rejection reason" className="mt-3 min-h-24 w-full rounded-lg border border-red-200 p-3 text-sm" placeholder="Required review reason" value={rejectionReason} onChange={(event) => setRejectionReason(event.target.value)} maxLength={1000} />
                <button type="button" onClick={() => void reject()} disabled={busy || !rejectionReason.trim()} className="mt-3 rounded-lg bg-red-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Reject with reason</button>
              </div>
            </div>
          )}
        </div>
