...
import {
  multipleAskFinalizeBrowserRequestSchema,
  multipleAskJobInternalResponseSchema,
  toBrowserJobResponse,
  toInternalFinalizeRequest,
...
export async function POST(request: NextRequest) {
  ...
  const parsed = multipleAskFinalizeBrowserRequestSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return jsonNoStore({ error: { code: "INVALID_REQUEST" } }, 400);
  try {
    const upstream = await callAiService("/api/v1/internal/multiple-ask/upload-sessions/finalize", "POST", toInternalFinalizeRequest(parsed.data), identity.uid, false, "multiple_ask", { requestId: parsed.data.requestId, accountTier: identity.accountTier });
